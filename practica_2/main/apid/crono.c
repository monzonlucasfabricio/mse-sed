...
int CRONO_GetSample(uint16_t i)
{
  return samples[i];
}
...
static void CRONO_timerCallback(void* arg)
{
  ...
  ESP_LOGI(TAG1,"%i",n);
   ...
   if (n < SAMPLES_SIZE)
   {
      ...
      sprintf(msg, "%d",samples[n] * 100 / 4096);
      sprintf(csvmsg, "%d,%d",n*100, samples[n] * 100 / 4096);
      MQTT_publish("p2/ldr_value", msg);
      MQTT_publish("p2/csv_value", csvmsg);
      ESP_LOGI(TAG1,"%s",msg);

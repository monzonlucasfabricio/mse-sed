...
/* Acciones a ejecutar para cada topic recibido */
   ...
   // Ingresar código aquí
    if(strcmp("p2/brilloled", topic)==0) 
	{
		/* Convert data to integer value */
		/* value -> 0 - 100 */
		uint8_t value = atoi(data);

		ESP_LOGI(TAG,"LED i: %d",value);

		/* Seteo el pwm */
		IO_pwmSet(value);
    }

    if(strcmp("p2/ldr_sampling",topic) == 0)
	{
    	if(strcmp(data,"grabar") == 0) 
		{
			ESP_LOGI(TAG,"GRABAR");
			grabar = 1;
		}
        if(strcmp(data,"finalizar") == 0) 
		{
			ESP_LOGI(TAG,"FINALIZAR");
			grabar = 2;
		}
	}
 }

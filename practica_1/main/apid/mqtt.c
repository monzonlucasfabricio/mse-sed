 ...
 ******************************************************************************/
 void MQTT_processTopic(const char * topic, const char * data){
   ...
   /* Acciones a ejecutar para cada topic recibido */
	if(strcmp("p1/led/toggle", topic) == 0) 
	{
		int estado;
		char msg[3];
        ESP_LOGI(TAG,"Mensaje recibido: %s", data);
        estado = IO_toggleLed();
		sprintf(msg, "%d", estado);
		ESP_LOGI(TAG,"Actualizo estado led : %d",estado);
		MQTT_publish("p1/led/estado",msg);
    }
	if(strcmp("p1/cap",topic) == 0)
	{
		if(strcmp(data,"1") == 0) capturar = true;
		if(strcmp(data,"0") == 0) capturar = false;
	}
 }
